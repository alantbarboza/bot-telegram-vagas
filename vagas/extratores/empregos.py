    ...
    for termo in termos_busca:
        for pagina in range(MAX_PAGINAS):
            ...
            try:
                ...
                info(f"Empregos | TERMO='{termo}' | PAGINA={pagina + 1} | VAGAS={total_cards} | URL={page.url}")
                ...
                for i in range(total_cards):
                    try:
                        ...
                        warning(f"Erro ao processar vaga Empregos | TERMO='{termo}' | PAGINA={pagina + 1} | URL={page.url} | ERRO={e}")
                        continue

                info(f"Empregos | TERMO='{termo}' | PAGINA={pagina + 1} | CAPTURADAS={vagas_capturadas}/{total_cards}")

            except Exception as e:
                warning(f"Erro ao acessar Empregos | TERMO='{termo}' | PAGINA={pagina + 1} | URL={page.url} | ERRO={e}")
                continue
