    ...
    for termo in termos_busca:
        ...
        for pagina in range(1, MAX_PAGINAS + 1):
            ...
            try:
                ...
                info(f"InfoJobs | TERMO='{termo}' | PAGINA={pagina} | VAGAS={total_cards} | URL={page.url}")
                ...
                for i in range(total_cards):
                    try:
                        ...
                        warning(f"Erro ao processar vaga InfoJobs | TERMO='{termo}' | PAGINA={pagina} | URL={page.url} | ERRO={e}")
                        continue

                info(f"InfoJobs | TERMO='{termo}' | PAGINA={pagina} | CAPTURADAS={vagas_capturadas}/{total_cards}")

            except Exception as e:
                warning(f"Erro ao acessar InfoJobs | TERMO='{termo}' | PAGINA={pagina} | URL={page.url} | ERRO={e}")
                continue
