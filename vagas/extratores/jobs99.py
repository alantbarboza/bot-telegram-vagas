    ...
    for termo in termos_busca:
        for pagina in range(1, MAX_PAGINAS + 1):
            ...
            try:
                ...
                page.wait_for_selector(".opportunities-list", timeout=15000)

                cards = elemento(page, ".opportunities-list .opportunity-card", todos=True)
                ...
                info(f"99Jobs | TERMO='{termo}' | PAGINA={pagina} | VAGAS={total_cards} | URL={page.url}")
                ...
                for i in range(total_cards):
                    try:
                        ...
                        warning(f"Erro ao processar vaga 99jobs | TERMO='{termo}' | PAGINA={pagina} | URL={page.url} | ERRO={e}")
                        continue

                info(f"99jobs | TERMO='{termo}' | PAGINA={pagina} | CAPTURADAS={vagas_capturadas}/{total_cards}")

            except Exception as e:
                warning(f"Erro ao acessar 99Jobs | TERMO='{termo}' | PAGINA={pagina} | URL={page.url} | ERRO={e}")
                continue
